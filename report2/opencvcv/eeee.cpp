...
int main()
{
	...
	cv::Mat Oct[12]; // 0 1 2 3 4 5~ 35     옥타브 W
	...
	for (int i = 0; i < 6; i++)
	{
		...
		//cv::GaussianBlur(image, Oct[i],cv::Size(GauO[i], GauO[i]) , 0, 0);
		cv::GaussianBlur(gray, Oct[6*j + i], cv::Size(sizeG, sizeG), Gau1[i], Gau1[i]);
		printf("1 : %d \n", Oct[0].cols);
		//0 옥타브 끝내기
	}
	num = num + 3;
	...
	for (int i = 1; i < 6; i++) //옥타브의 첫번째꺼는 그전에 저장해놨으므로
	{
		...
		double currentSig = sqrt(pow(Sig, 2)) - pow((beforeSig), 2); //가우시안을 빨리 처리하기 위해
		cv::GaussianBlur(Oct[6 * j + i - 1], Oct[6*j + i], cv::Size(sizeG, sizeG), Sig, Sig);//차이로 스무딩
		printf("2 : %d \n", Oct[6].cols);
		...
		//1 옥타브 끝내기
											...
											//옥타브 2개로 구현 끝
														...
														//DOG 구현 시작 
	cv::Mat DOG[10];//j개의 옥타브는 j*5개의 DOG로 표현가능. 여기서 j는 2이므로 10개의 DOG
	int count = 0;
					...
					for (int i1 = 0; i1 < DOG[((q * 5) + i)].cols; i1++) //가로
					{
						...
						else
						{
							...
							if (isMax(check,27)) //최대값이라면
							{
								...
								//printf("최대로 검출!!!!! \n");
								cv::circle(image, cv::Point(i1*pow(2,q), j1*pow(2,q)), 1.6*pow(2, (q + i)/ 3.0), cv::Scalar(0,255,0), 1);
